...
import { BOT_CHAT, POST_LEARNING_TIME } from "../../../config/backend_endpoints";
...
const LearnSubjectChat = () => {
  ...
  const { user, token, logoutUser, users_children, selected_Child_index} = useAppContext();
  const date = new Date();
  // 2024-02-06 08:40
  const [startTime, setStartTime] = useState(`${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}`);
  ...
  useEffect(()=>{
    return ()=>{
      ...
      // console.log("Hey component end");
      postTime(authorizedAxios, users_children[selected_Child_index].id, startTime);
    }
  }, []);
          ...
          >
            ...
            >
              <CustomBreadcrumbs
                data={[
                  ...
                  {
                    path: ROUTES.PARENT.LEARN_SUBJECT.DETAIL.replace(':subjectId',subject.id),
                    title: subject?.subjectName || 'N/A',
                  },
                  {
                    path: ROUTES.PARENT.LEARN_SUBJECT.CHAT.replace(
                      ':subjectId',
                      subject.id
            ...
            >
              ...
              <Box sx={{ width: "100%" }}>
                ...
                >{` ${course?.courseName || "N/A"} `}</Typography>
                {/* <Typography
                  sx={{
                    fontSize: $({ size: 31.98 }),
                    fontWeight: "600",
            ...
            >
              <Box sx={{ height: $({ size: 500, numeric: true }), overflow: "scroll", p: 2 }}>
                {messages.map((message) => (
                  <Message key={message.id} message={message} />
                ))}
                { serverRequest.loading && 
                    <Message key="BotThinking" loading={serverRequest.loading} message={{ id: "BotThinking", text: "", sender: "bot" }} />  
                }
                {/* <TypingDots /> */}
...
const chatCommunication = async (authorizedAxios, setServerRequest, messagesData, setMessages, data) => {
  setServerRequest({
    loading: true,
    status: "start",
    response: "",
...
const postTime = async (authorizedAxios, childId, startTime) => {
  
  try {
    const date = new Date();
    const response = await authorizedAxios.post(POST_LEARNING_TIME, {
      childId:childId,
      "start_datetime": startTime,
      "end_datetime": `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}`
                      });
...
export default LearnSubjectChat;
